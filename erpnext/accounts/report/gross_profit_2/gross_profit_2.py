...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		"parent": _("Sales Invoice") + ":Link/Sales Invoice",
...
class GrossProfitGenerator(object):
	...
	def process(self):
		...
		for item_group in self.si_list:
			self.grouped.setdefault(item_group[0].get(scrub(self.filters.group_by)), [])
			for row in item_group:
				...
				# add to grouped
				self.grouped[row.get('item_group')].append(row)
	...
	def get_average_rate_based_on_group_by(self):
		...
		for j, key in enumerate(self.grouped):
			self.grouped_data.append([])
			if self.filters.get("group_by") != "Invoice":
				for i, row in enumerate(self.grouped[key]):
					new_row = row
					new_row = self.set_average_rate(new_row)
					self.grouped_data[j].append(new_row)
			else:
	...
	def load_invoice_items(self):
		def get_sales_inv(self, from_date, to_date):
			doc = frappe.get_last_doc("Sales Invoice")
			conditions = ""
			if self.filters.company:
				conditions += " and company = %(company)s"
			if from_date:
				conditions += " and `tabSales Invoice`.posting_date >= '{0}'".format(from_date)
			if to_date:
				conditions += " and `tabSales Invoice`.posting_date <= '{0}'".format(to_date)
			...
			return si_list_item
		from1 = datetime.strptime(self.filters.from_date, '%Y-%m-%d').date()
		to1 = datetime.strptime(self.filters.to_date, '%Y-%m-%d').date()
